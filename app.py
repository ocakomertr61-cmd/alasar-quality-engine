    ...
    def _veritabanini_hazirla(self):
        ...
        try:
            if not os.path.exists(self.dosya_adi):
                ...
                self._ekrana_yaz(f"[SİSTEM] Excel veritabanı başarıyla oluşturuldu: {self.dosya_adi}")
        except Exception as e:
            self._ekrana_yaz(f"[KRİTİK HATA] Excel dosyası oluşturulamadı! Hata: {e}")

    def _ekrana_yaz(self, mesaj):
        """Terminalin kilitlenmesini önleyerek mesajı anında ekrana basar."""
        print(mesaj)
        sys.stdout.flush()

    def yeni_kayit_ekle(self, sirket, ref_no, ph, miktar, neden, is_tanimi, talep_tarihi, onay_tarihi="-", durum="Onaylandı"):
        """Sisteme yeni bir takip kaydı ekler, saat formülünü işletir ve kaydeder."""
        ...
        try:
            ...
            self._ekrana_yaz("[UYARI] pH veya Miktar sayısal değil! Saat 0 kabul edildi.")
            talep_edilen_saat = 0.0

        # --- EXCEL OKUMA VE YAZMA (KİLİTLENME KORUMALI) ---
        try:
            ...
            df.to_excel(self.dosya_adi, index=False)
            self._ekrana_yaz(f"[BAŞARILI] {yeni_id} oluşturuldu ({sirket}). Hesaplanan Saat: {talep_edilen_saat}")
            return True
        except PermissionError:
            self._ekrana_yaz(f"[KİLİTLENME HATASI] '{self.dosya_adi}' dosyası şu an Excel programında açık! Lütfen Excel'i kapatıp kodu tekrar çalıştırın.")
            return False
        except Exception as e:
            self._ekrana_yaz(f"[HATA] Veri yazılamadı: {e}")
            return False

    def durum_guncelle(self, kayit_id, yeni_durum):
        """Mevcut bir kaydın son durumunu güvenli bir şekilde günceller."""
        ...
        try:
            ...
            if kayit_id in df["Kayit_ID"].values:
                ...
                df.to_excel(self.dosya_adi, index=False)
                self._ekrana_yaz(f"[GÜNCELLENDİ] {kayit_id} durumu '{yeni_durum}' yapıldı.")
                return True
            else:
                self._ekrana_yaz(f"[HATA] {kayit_id} bulunamadı!")
                return False
        except PermissionError:
            self._ekrana_yaz(f"[KİLİTLENME HATASI] Güncelleme yapılamıyor, Excel dosyası açık!")
            return False

    def verileri_goster(self):
        """Mevcut tüm tabloyu dataframe olarak döndürür."""
        try:
...
if __name__ == "__main__":
    ...
    motor.yeni_kayit_ekle(
        ...
        ph="5.5",
        miktar=1250,
        neden="Kalıp yüzeyinde çapaklanma",
        is_tanimi="Ekstra çapak temizleme işlemi",
    ...
    motor.yeni_kayit_ekle(
        ...
        ph="7.2",
        miktar=500,
        neden="Müşteri revizyon talebi (Legrand)",
        is_tanimi="Montaj hattında buton basmama sorunu çözümü",
        talep_tarihi="2026-05-14",
        onay_tarihi="2026-05-15",
        durum="Onaylandı"
    )

    motor._ekrana_yaz("\n--- 2. ADIM: VERİ DURUMU VE FORMÜL SONUÇLARI ---")
    tablo = motor.verileri_goster()
    if not tablo.empty:
        print(tablo[["Kayit_ID", "Şirket", "Talep_Edilen_Saat", "Son_Durum"]])
        sys.stdout.flush()

    motor._ekrana_yaz("\n--- 3. ADIM: GÜNCELLEME İŞLEMİ ---")
    ...
    if not tablo_son.empty:
        print(tablo_son[["Kayit_ID", "Şirket", "Talep_Edilen_Saat", "Son_Durum"]])
        sys.stdout.flush()
