...
# Veritabanı Sütun Yapısını Güncelledik (Yönetici Aksiyonu eklendi)
if 'denetim_gecmisi' not in st.session_state:
    st.session_state.denetim_gecmisi = pd.DataFrame(columns=[
        ...
        "TRI", "HKY", "Sistem Kararı", "Yönetici Aksiyonu", "Yönetici Notu"
    ])
...
def kalite_motoru_hesapla(F3, G3, J3, K3, L3, M3, P3_p, Q3_p, R3_p, S3_p):
    ...
    # KESİN RED ŞARTLARI (%5 HKY BURADA)
...
# --- UI ---
st.set_page_config(page_title="Alasar Quality V4.2", layout="wide")
st.title("🛡️ Alasar Quality Engine V4.2")
...
res1.metric("TRI (Risk)", f"{t3_skor:.4f}")
res2.metric("HKY (Hata Oranı)", f"%{hky_sonuc:.2f}")
res3.header(f"{ikon} {karar}")

# --- DİNAMİK YÖNETİCİ PANELİ (SARI ve RED Durumlarında Aktif) ---
final_aksiyon = "OTOMATİK ONAY"
admin_note = "-"
is_authorized = True 

if karar in ["SARI", "RED"]:
    is_authorized = False # Karar verilene kadar kayıt kapalı
    st.sidebar.divider()
    st.sidebar.subheader(f"🔐 Yönetici {karar} Yönetimi")
    
    # Karar Seçeneklerini Belirle
    if karar == "SARI":
        aksiyon_listesi = ["Şartlı Kabulü Onayla ✅", "Kesin Reddet ❌"]
    else: # RED DURUMU
        aksiyon_listesi = ["%100 Ayıklama Yapılsın 🔍", "Karantina / Hurda 📦", "Tedarikçiye İade 🚛"]
        st.sidebar.error("SİSTEM RED VERDİ! Aksiyon Seçiniz.")

    admin_choice = st.sidebar.radio("Kararınız / Aksiyonunuz:", aksiyon_listesi)
    pwd_input = st.sidebar.text_input("Yönetici Parolası", type="password")
    admin_note = st.sidebar.text_area("Yönetici Karar Notu")

    if pwd_input == ADMIN_PASSWORD:
        is_authorized = True
        final_aksiyon = admin_choice
    elif pwd_input != "":
        st.sidebar.error("Hatalı Parola!")
        st.session_state.error_count += 1
        if st.session_state.error_count >= 3:
            st.sidebar.warning(f"🚨 KRİTİK: {TARGET_EMAIL} adresine mail gönderildi!")
            st.session_state.error_count = 0
    else:
        st.warning(f"⚠️ {karar} DURUMU: Kayıt için yönetici onayı şarttır.")

# --- KAYDET BUTONU ---
if st.button("💾 DENETİMİ VERİTABANINA KAYDET", use_container_width=True):
    if is_authorized:
        yeni_kayit = {
            ...
            "Yönetici Aksiyonu": final_aksiyon,
        ...
        st.success(f"Başarıyla Kaydedildi: {final_aksiyon}")
    else:
        st.error("Yönetici şifresi girilmeden bu kayıt yapılamaz!")

# --- LİSTELEME ---
st.divider()
st.subheader("📜 Denetim Geçmişi ve Yönetici Aksiyonları")
if not st.session_state.denetim_gecmisi.empty:
    ...
    st.download_button("📥 Excel Olarak İndir", csv, "alasar_kalite_denetim.csv", "text/csv")
